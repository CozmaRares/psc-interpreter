...
const char IDENTIFIER_CHARS[] = "_0123456789AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZz";
...
const char T_NULL = 0,
           ...
           T_MODULO = 6,
           T_STRING = 7,
           T_END_LINE = 8,  // last token after lexer finishes, helpful for parsing
    T_AND = 9,
           T_OR = 10,
           T_CHAR = 11,
           T_KEYWORD = 12,
...
const string* VALUE_NAMES_ARRAY[] = {
    &VALUE_NAMES_MAP["NULL"],
    &VALUE_NAMES_MAP["NUMBER"],
    &VALUE_NAMES_MAP["CHAR"],
    &VALUE_NAMES_MAP["ARRAY"],
    &VALUE_NAMES_MAP["STRING"],
    &VALUE_NAMES_MAP["DICTIONARY"],
...
struct Error {
    ...
    Error() {}
    Error(string name_, string details_, Position pos_start_, Position pos_end_) : name(name_), details(details_), pos_start(pos_start_), pos_end(pos_end_) {}
    ...
    void print_error() {
        ...
        cout << ERRORS["File"] << " <" << LINES[pos_start.line].file_name << ">, " << ERRORS["line"] << ' ' << LINES[pos_start.line].line << '\n';
...
string double_to_string(double n) {
    ...
    if (dot_pos != string::npos) {
        ...
        while (output[pos] == '0' && pos >= dot_pos) pos--;
