...
struct Parser {
    ...
    Parse_result bin_op(function<Parse_result()> f, vector<char> op) {
        ...
        Node_shared_ptr stanga = rez.node;
        ...
        while (current_token != T_END_LINE) {
            ...
            stanga = make_shared<AST_binary_operation_node>(stanga, operation, rez.node);
        }
    ...
    Parse_result make_array() {
        ...
        if (tokens[pos + 1] == T_BOX_BRACKET_RIGHT) {
            advance();
            return Parse_result(make_shared<AST_array_node>(pos_start, current_token->pos_end, vector<Node_shared_ptr>()));
        }
    ...
    Parse_result make_dict() {
        ...
        if (tokens[pos + 1] == T_CURLY_BRACKET_RIGHT) {
            advance();
            return Parse_result(make_shared<AST_dictionary_node>(pos_start, current_token->pos_end, vector<Node_shared_ptr>(), vector<Node_shared_ptr>()));
        }
    ...
    Parse_result make_body(vector<string> end_words) {
        ...
        do {
            if (current_token != T_END_LINE)
                return Error(ERRORS["Parsing error"], ERRORS["End of line expected"], current_token->pos_start, current_token->pos_end);

            if (in == cin_stream)
                cout << MAP_FILE_NAMES_TO_NO_LINES[PROGRAM_NAME] + 1 << " ... ";

            if (!((*in) >> input))
                return Error(ERRORS["Parsing error"], ERRORS["Reached end of file and not all bodies are closed"], Position(LINES.size() - 1, 0), Position(LINES.size() - 1, LINES.back().text.size()));
    ...
    Parse_result make_if() {
        ...
        if (current_token != T_KEYWORD || current_token->value() != KEYWORDS["then"])
            return Error(ERRORS["Parsing error"], ERRORS["Expected"] + ' ' + KEYWORDS["then"], current_token->pos_start, current_token->pos_end);

        advance();

        Parse_result body = make_body({ "else", "end" }), else_body = nullptr;
        ...
        if (current_token != T_KEYWORD || current_token->value() != KEYWORDS["end"])
            return Error(ERRORS["Parsing error"], ERRORS["Expected"] + ' ' + KEYWORDS["end"], current_token->pos_start, current_token->pos_end);
    ...
    Parse_result make_for() {
        ...
        Parse_result assignment = tokens[pos + 1] == T_BOX_BRACKET_LEFT ? make_assignment_index(false) : make_assignment_variable(false);
...
/*
        ...
        Parse_result assignment = expression();
...
*/
      ...
      //  assignment.node = make_shared<AST_variable_assign_node>(variable, assignment.node, false);

        advance();

        Parse_result stop = expression(), skip = nullptr;
        ...
        if (current_token != T_KEYWORD || current_token->value() != KEYWORDS["execute"])
            return Error(ERRORS["Parsing error"], ERRORS["Expected"] + ' ' + KEYWORDS["execute"], current_token->pos_start, current_token->pos_end);
    ...
    Parse_result make_do_until() {
        ...
        if (current_token != T_KEYWORD || current_token->value() != KEYWORDS["until"])
            return Error(ERRORS["Parsing error"], ERRORS["Expected"] + ' ' + KEYWORDS["until"], current_token->pos_start, current_token->pos_end);
    ...
    Parse_result base() {
        ...
        if (current_token == T_NUMBER) {
            ...
            if (keyword == KEYWORDS["if"])
                ...
                return make_shared<AST_return_node>(node.node);
            } else if (keyword == KEYWORDS["break"]) {
                Position p1 = current_token->pos_start, p2 = current_token->pos_end;
                advance();
                return make_shared<AST_break_continue_node>(true, p1, p2);
            } else if (keyword == KEYWORDS["continue"]) {
                Position p1 = current_token->pos_start, p2 = current_token->pos_end;
                advance();
                return make_shared<AST_break_continue_node>(false, p1, p2);
            } else if (keyword == KEYWORDS["include"]) {
                advance();
                if (current_token != T_STRING)
                    return Error(ERRORS["Parsing error"], ERRORS["Expected"] + ' ' + *VALUE_NAMES_ARRAY[V_STRING], current_token->pos_start, current_token->pos_end);

                Token_shared_ptr t = current_token;
                advance();
                return make_shared<AST_include_node>(t);
                ...
                Token_shared_ptr t = current_token;
                advance();
                return make_shared<AST_run_node>(t);
            } else
                return nullptr;
    ...
    Parse_result comparison() { return bin_op(bind(&Parser::term, this), { T_EQUALS, T_LESS, T_LESS_EQUAL, T_GREATER, T_GREATER_EQUAL, T_DIFFERENT }); }
    ...
    Parse_result expression() {
        ...
        if (current_token == T_IDENTIFIER) {
            ...
            Parse_result res = tokens[pos + 1] == T_BOX_BRACKET_LEFT ? make_assignment_index(is_const) : make_assignment_variable(is_const);
