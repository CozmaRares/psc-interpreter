...
struct RT_Error {
    ...
    RT_Error() {}
    RT_Error(Context* context_, string details_, Position pos_start_, Position pos_end_) : context_name(context_->name), details(details_), pos_start(pos_start_), pos_end(pos_end_) {}
    ...
    void print_error() {
...

        {
            ...
            if (context_name != a.first || pos_start.line != a.second)
                cout << "   " + ERRORS["In"] + " <" << context_name << ">, " + ERRORS["line"] + ' ' << LINES[pos_start.line].line << '\n';
        }

        int i;

        for (i = PARENT_CONTEXTS.size() - 1; i >= 0; i--)
            cout << "   " + ERRORS["In"] + " <" << PARENT_CONTEXTS[i].first << ">, " + ERRORS["line"] + ' ' << LINES[PARENT_CONTEXTS[i].second].line << '\n';
...
struct Predefined_function_value : public Function_base_value {
    ...
    Interpreter_result execute_locals() {
        Dictionary_value dict(map<string, Value_shared_ptr>(), false);

        for (auto& var : context->memory.variable_map)
            dict.m['"' + var.first + '"'] = var.second->copy();

        for (auto& file : context->in_files)
            dict.m['"' + file.first + '"'] = make_shared<String_value>(ERRORS["File"] + " -> " + BUILT_INS["file mode read"] + file.second.path);

        for (auto& file : context->out_files)
            dict.m['"' + file.first + '"'] = make_shared<String_value>(ERRORS["File"] + " -> " + *file.second.mode + ' ' + file.second.path);
    ...
    Interpreter_result execute_close_file(Value_shared_ptr identifier) {
        if (identifier != V_STRING)
            return RT_Error(context, ERRORS["Expected"] + ' ' + VALUE_NAMES_MAP["STRING"], identifier->pos_start, identifier->pos_end);
