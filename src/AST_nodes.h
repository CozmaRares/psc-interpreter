...
struct AST_dictionary_node : public AST_node {
    ...
    AST_dictionary_node(Position pos_start_, Position pos_end_, vector<Node_shared_ptr> keys_, vector<Node_shared_ptr> values_)
        ...
        for (int i = 0; i < keys.size(); i++) {
            ...
            switch (key.value->type) {
                ...
                default: {
                    ...
                    details << ERRORS["Expected"] << ' ' << VALUE_NAMES_MAP["NUMBER"] << ", " << VALUE_NAMES_MAP["CHAR"] << ", " << VALUE_NAMES_MAP["STRING"];
...
struct AST_index_assign_node : public AST_node {
    ...
    AST_index_assign_node(Token_shared_ptr identifier_, Node_shared_ptr arr_, Node_shared_ptr index_, Node_shared_ptr value_, bool is_const_)
...
struct AST_for_node : public AST_node {
    ...
    Interpreter_result visit(Context* context) override {
        ...
        if (start.value != V_NUMBER)
            return RT_Error(context, ERRORS["Expected"] + ' ' + VALUE_NAMES_MAP["NUMBER"], start_node->pos_start, start_node->pos_end);

        if (stop.is_error())
            return stop;

        if (stop.value != V_NUMBER)
            return RT_Error(context, ERRORS["Expected"] + ' ' + VALUE_NAMES_MAP["NUMBER"], stop_node->pos_start, stop_node->pos_end);
        ...
        if (skip_node == nullptr)
            ...
            if (skip.value != V_NUMBER)
                return RT_Error(context, ERRORS["Expected"] + ' ' + VALUE_NAMES_MAP["NUMBER"], skip_node->pos_start, skip_node->pos_end);
        }
...
struct AST_function_call_node : public AST_node {
    ...
    Interpreter_result visit(Context* context) override {
        // if (APELE_FUNCTII > 100)
        //    return cEroare_rulare_v("overflow", pozStart, pozFinal, context);
...
struct AST_return_node : public AST_node {
    ...
    Interpreter_result visit(Context* context) override {
        ...
        return Interpreter_result(res.value, RT_Error(context, ERRORS["Cannot return values outside of functions"], ret->pos_start, ret->pos_end));
    }
};
